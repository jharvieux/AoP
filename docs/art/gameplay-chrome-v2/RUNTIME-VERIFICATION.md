...
This report binds the #610 runtime implementation to the approved Direction B checkpoint and records refreshed exact-source runtime evidence after the #612 city-shell change. The operator renewed approval for the prior three exact-source runtime frames at `e547b55ced81d5f45fb110bf0b95a070b0c48274`; that [historical approval](https://github.com/jharvieux/AoP/issues/610#issuecomment-5548488129) cannot carry across the material #612 change. The three current candidates were captured against source head `45a206f760eacce50dc8dd1dc656c5d4e789cb3c` and await renewed exact-byte approval.
...
The refreshed candidate projects to a conservative schema-v3 boundary containing **215 shipping source files**: 149 non-test files recursively discovered under `apps/web/src`, all 29 engine source files, all 19 shared source files and all 18 content source files. It separately binds **22 web/package build inputs**, including the HTML entry, PWA worker/manifest/icons, workspace/dependency resolution, package exports, TypeScript/Vite configuration and Vite helpers. Exact sorted inventories make an added, removed or renamed shipping file fail, while exact byte counts and SHA-256 values catch content changes in the active shell, theme resolver, helpers, Canvas components and package leaves. Until renewed approval updates the immutable anchor, the retained `RUNTIME-CAPTURE-BINDINGS.json` intentionally remains the prior approved manifest and the normal builder rejects this changed projection.
...
## Refreshed runtime capture candidates — approval pending

The supervisor rebuilt the production preview after the source freeze, waited for the runtime to settle, and native-inspected every frame for font fallback or incomplete or missing imagery. No programmatic font/image await is claimed. These are the resulting real runtime candidates:
...
Native-size inspection found no font fallback, missing image, clipped label, control below 44 × 44, or unintended opaque panel over a selectable map entity. These files are exact-source candidates, not yet approved runtime evidence. The immutable material baseline intentionally remains on the prior approved head, so the builder and validator fail closed until renewed operator approval is recorded and a separate manual anchor patch binds the current source, capture bytes, and canonical approval record.
