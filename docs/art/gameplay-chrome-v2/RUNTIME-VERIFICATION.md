...
Native-size inspection found no font fallback, missing image, clipped label, control below 44 × 44, or unintended opaque panel over a selectable map entity. The operative carry-forward boundary is the complete schema-v3 source-tree inventory, build inputs, full stylesheet, runtime receipts/assets, frozen capture states, and capture bytes; the older scoped CSS projections are diagnostic only. The operator approval carries across docs/evidence-only commits only while that complete manifest remains exact; any material bound-byte or inventory change requires renewed capture approval.
