...
This report binds the #610 runtime implementation to the approved Direction B checkpoint and the refreshed exact-source runtime evidence after the #612 city shell, #613 map UX, and #623 single-player fog-privacy changes. The operator approved all three current exact-source runtime frames in the active session on 2026-09-05 after reviewing evidence head `08717289778883d7adca6ff7a6f15b20fb7c6b25`. The files were captured from runtime/source ancestor `c1824f22bf14dca6d38f7519fd99affd789a8130`; their permanent issue-comment record and separate immutable material-anchor renewal remain pending.
...
- Web tests: 108 files, 1,007 tests passed.
...
The isolated writer's first literal wrapper attempt could not replace read-only dependency links and exited with `ERR_PNPM_ABORTED_REMOVE_MODULES_DIR_NO_TTY`; the equivalent repository-owned commands passed there. Independent exact-source verification later ran the literal `CI=true pnpm verify` gate successfully. The final exact-source gate passes Prettier, all five TypeScript projects, 108 web files / 1,007 tests, 37 engine files / 765 tests, 5 shared files / 57 tests, and the production build.

## Exact-source capture binding

The approved candidate projects to a conservative schema-v3 boundary containing **218 shipping source files**: 152 non-test files recursively discovered under `apps/web/src`, all 29 engine source files, all 19 shared source files and all 18 content source files. It separately binds **22 web/package build inputs**, including the HTML entry, PWA worker/manifest/icons, workspace/dependency resolution, package exports, TypeScript/Vite configuration and Vite helpers. Exact sorted inventories make an added, removed or renamed shipping file fail, while exact byte counts and SHA-256 values catch content changes in the active shell, theme resolver, helpers, Canvas components and package leaves. The retained binding intentionally remains on the prior approved anchor until the permanent issue record and separate manual anchor patch land.
...
`RUNTIME-MATERIAL-BASELINE.json` closes the generator trust gap: it is a non-generated anchor containing an exact evidence head, a SHA-256 identity for the external capture record and the immutable SHA-256/byte count/census of the complete schema-v3 material projection. The normal builder recomputes and compares that projection before writing `RUNTIME-CAPTURE-BINDINGS.json`; it cannot update the anchor. The current preliminary promotion deliberately leaves the prior anchor in place, so the builder stops with material-baseline drift until a permanent issue-comment record exists and the distinct manual anchor patch binds this exact promotion commit.
...
## Session-approved runtime capture evidence — durable record pending

The supervisor rebuilt the production preview after the source freeze, waited for the runtime to settle, and native-inspected every frame for font fallback or incomplete or missing imagery before approval. No programmatic font/image await is claimed. These are the exact real runtime frames approved in the active session and promoted byte-for-byte here:

| File                                                                                                  | Dimensions | PNG color type |   Bytes | SHA-256                                                            |
| ----------------------------------------------------------------------------------------------------- | ---------: | -------------: | ------: | ------------------------------------------------------------------ |
| [`runtime-phone-world-375x812.png`](runtime-captures/runtime-phone-world-375x812.png)                 |  375 × 812 |        2 (RGB) | 137,822 | `cb6b686837c2258682f54936e92fd777df1a3b29787c4318293ec68468ba92a5` |
| [`runtime-city-inspector-1440x900.png`](runtime-captures/runtime-city-inspector-1440x900.png)         | 1440 × 900 |    3 (indexed) | 247,242 | `f03d02ff4cd42f44cdf83702155bd512d646c999dfa4fcc3bf1694ce90c17ad9` |
| [`runtime-interaction-states-1440x960.png`](runtime-captures/runtime-interaction-states-1440x960.png) | 1440 × 960 |    3 (indexed) | 293,704 | `9ab69ff52ef5cfbb308a254832be581f668f76fc20b9e9251a49d5a96756af55` |

The in-app Browser returned JPEG/JFIF payloads, which were decoded and saved twice byte-identically as real PNGs at unchanged dimensions: the phone frame as truecolor RGB, and the two large city frames as indexed PNGs without dithering so they remain below the evidence budget. The phone's decoded RGB is exact; the indexed frames have measured RGB RMSE 9.262423 and 9.759014. The #612 RGB/JFIF capture set remains the city-art fidelity evidence. These three PNGs total **678,768 bytes**; the largest is 293,704 bytes, so every review capture remains below 300 KiB and no byte belongs to the shipping runtime bundle.
...
Native-size inspection found no font fallback, missing image, clipped label, control below 44 × 44, or unintended opaque panel over a selectable map entity. The permanent issue #610 record is still pending; until it exists and the separate manual material-anchor patch lands, the normal builder and validator intentionally fail closed rather than treating the in-session approval as a durable record.
