...
- `RUNTIME-MATERIAL-BASELINE.json` is the non-generated material anchor for the recorded `e547b55` capture head. It holds the immutable schema-v3 material-projection digest and the capture-record identity hash, so the normal builder can validate this baseline but cannot refresh it.
