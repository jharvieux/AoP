...
This directory contains the approved visual contract and deterministic review package for [issue #610](https://github.com/jharvieux/AoP/issues/610). The operator approved **Direction B — Gilded Harbor Diorama with the recommended city refinement** before runtime implementation began and approved the three current exact-source runtime frames in the active session on 2026-09-05. The frames were captured from runtime/source ancestor `c1824f22bf14dca6d38f7519fd99affd789a8130` and reviewed at evidence head `08717289778883d7adca6ff7a6f15b20fb7c6b25`; their byte-exact promotion is committed here while the permanent issue-comment record and immutable material-anchor renewal remain pending. The mockups remain non-runtime evidence; the implemented CSS, components, vector primitives, and verification are recorded in [RUNTIME-VERIFICATION.md](RUNTIME-VERIFICATION.md).
...
- `RUNTIME-MATERIAL-BASELINE.json` remains the non-generated material anchor for the prior approved evidence until a permanent issue-comment URL records the 2026-09-05 approval. It holds the immutable schema-v3 material-projection digest and the capture-record identity hash, so the normal builder can validate an approved baseline but cannot refresh it.
...
- `build-runtime-capture-bindings.mjs` regenerates that boundary from the repository trees only after the material projection matches the fixed anchor. Any added, removed, renamed, or changed bound file stops the builder before it can rewrite the binding. The older selector projections remain diagnostics only; the full `styles.css` hash is acceptance evidence so broad selectors, cascade changes, keyframes, and previously unreferenced Canvas tokens cannot escape.
- `renew-runtime-capture-baseline.mjs` is proposal-only. It requires a completely clean tracked and untracked worktree; equality across local HEAD, its same-named upstream, the hosted branch and the hosted PR head; and exact `git show HEAD:path` parity for every material file. It resolves only a canonical live issue #610 comment whose API URL/issue identity match and whose author association is `OWNER`, but treats the opaque comment body only as evidence for human review. It prints the comment URL, author, body hash, source/material/capture hashes and unchanged before/after anchor/binding hashes. It has no file-writing capability and cannot renew or re-bless evidence.
- Renewal remains a manual trust decision: after direct trusted-user approval, a reviewer must compare the opaque comment body to every printed fact, then make a separately reviewed manual `apply_patch` to `RUNTIME-MATERIAL-BASELINE.json`. The normal builder and validator continue to fail until that distinct patch supplies the newly approved anchor; a GitHub comment or proposal output is never autonomous authorization.
...
Before the permanent approval record and separate manual anchor patch land, run `node compose.mjs` and expect the normal binding builder/validator to reject the stale material baseline. After that renewal, run `node build-runtime-capture-bindings.mjs` twice and `node validate.mjs`; then run repository formatting if the generated JSON layout changes.
...
The operator approved the three current exact-source runtime frames in the active session on 2026-09-05 after reviewing the exact evidence at `08717289778883d7adca6ff7a6f15b20fb7c6b25`. They were captured from runtime/source ancestor `c1824f22bf14dca6d38f7519fd99affd789a8130` and are byte-exact with candidate digest `86dc0f7afa29a73c3983942d39a43d7dd9a0e681189216457374d93fcd59e3db`. The permanent issue #610 comment URL and separate manual material-anchor patch are still pending, so the retained baseline intentionally remains stale and the normal builder continues to fail closed.
