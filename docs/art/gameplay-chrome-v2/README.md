...
- `renew-runtime-capture-baseline.mjs` is the separate, explicit renewal path. It requires a new exact capture head, a new issue-record URL, the capture date, the exact `RENEW_CAPTURE_APPROVAL` confirmation, and matching head/record/date text in both review documents before it atomically replaces the anchor and binding. The script restores both prior files if regeneration fails.
