...
This directory contains the approved visual contract and deterministic review package for [issue #610](https://github.com/jharvieux/AoP/issues/610). The operator approved **Direction B — Gilded Harbor Diorama with the recommended city refinement** before runtime implementation began, then approved all three refreshed exact-source runtime frames on 2026-09-05. The frames were captured from runtime/source ancestor `c1824f22bf14dca6d38f7519fd99affd789a8130`, reviewed at preliminary evidence head `08717289778883d7adca6ff7a6f15b20fb7c6b25`, and promoted byte-identically at evidence head `ec86cebf1d0d4c70c2a670f142bdddf6fab265ee`. The approval is recorded in [issue #610](https://github.com/jharvieux/AoP/issues/610#issuecomment-5555063774). The mockups remain non-runtime evidence; the implemented CSS, components, vector primitives, and verification are recorded in [RUNTIME-VERIFICATION.md](RUNTIME-VERIFICATION.md).
...
- `RUNTIME-MATERIAL-BASELINE.json` is the non-generated material anchor for the recorded `ec86ceb` evidence head. It holds the immutable schema-v3 material-projection digest and the capture-record identity hash, so the normal builder can validate this baseline but cannot refresh it.
...
Run `node compose.mjs`, `node build-runtime-capture-bindings.mjs` twice, and `node validate.mjs` to recreate and verify the proof and approval-bound runtime evidence. Run repository formatting if the generated JSON layout changes.
...
The operator approved all three refreshed exact-source runtime frames promoted at `ec86cebf1d0d4c70c2a670f142bdddf6fab265ee`; the approval is recorded in [issue #610](https://github.com/jharvieux/AoP/issues/610#issuecomment-5555063774). The files were captured from runtime/source ancestor `c1824f22bf14dca6d38f7519fd99affd789a8130` and are byte-exact with candidate digest `86dc0f7afa29a73c3983942d39a43d7dd9a0e681189216457374d93fcd59e3db`. That approval carries across docs/evidence-only commits only while `RUNTIME-CAPTURE-BINDINGS.json` proves the complete schema-v3 source-tree inventory, build inputs, full stylesheet, runtime receipts/assets, frozen capture states, and capture bytes remain exact; any material bound-byte or inventory change requires renewed capture approval.
