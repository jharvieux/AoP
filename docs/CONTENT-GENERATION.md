...
2. **Generate the clip:**
   ...
   python ~/aop-ai-tools/generate-game-audio-piper.py native \
     ...
     --filename native_fight_taunt.wav   # or pass positional args per the script's own usage
   ```

   (See `docs/runbooks/audio-generation.md` for the exact current CLI shape — the script
   takes `<character> <text>` and derives the filename, or accepts a batch dict for
   predefined lines.)
