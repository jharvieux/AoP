...
/**
 * Interactive-combat planner (#93, #305, #344). The probe helpers that drive
 * the boarding melee, naval-tactics rounds, and city assault now live in
 * `@aop/engine` (`probe.ts`) so single-player and the multiplayer server run
 * the identical pure simulation. This module keeps the UI-side command
 * builders the sheets tap through — `planMove`, `planAttack`, `stackLosses` —
 * and re-exports the moved probes so existing imports stay put.
 */
